function [G_LPM,THz] = MRSIMOLPMOpenLoop(u,y,n,R,P,nT)
...
Nn = floor(Np/2)+1; % amount of samples per period up to nyquist
...
Yk1 = Yf(1:P*Nn,:)'; % up to nyquist frequency (!!!!???) ESSENTIAL
Uk1 = Uf(1:P*Nn,:)';
Zk = [Yk1;Uk1];       % Pintelon 2012 (7-48)
...
Zkh = Zk(1:Ny,1:P:end)-THz(1:Ny,:);

% sort data records for MISO parameterization
Uk2 = Uk1(:,1:P:end)-THz(Ny+1:end,:);
Uk = zeros(1,length(Uk2));
for i = 1:F
    Uk(i:F:end) = Uk2(i,i:F:end);
end
